...
-type connection() ::
    #{socket := gen_tcp:socket() | ssl:sslsocket(),
      ...
      decode_state := cow_hpack:state(),
      encode_state := cow_hpack:state(),
      last_stream := integer(),
        ...
        {ok, Frame} ->
            send_settings(Frame, 
                          Connection#{client_settings => AllSettings,
                                      decode_state => cow_hpack:init(HeaderTabSize)
                                     });
        {error, Error} ->
...
acknowledge(#{server_settings := #{header_table_size := ServerHeaderTabSize}} = Connection) ->
    ...
    case send(Connection, AckFrame) of
        ok ->
            receive_ack(Connection#{encode_state => cow_hpack:init(ServerHeaderTabSize)});
        {error, Error} ->
            send_goaway(Connection, ?PROTOCOL_ERROR, Error),
...
handle_headers([?END_HEADERS | OtherFlags], StreamId, Data, _Size, 
    ...
    {Headers, NewDecodeState} = cow_hpack:decode(Data, DecodeState),
...
do_send_headers(#{encode_state := EncodeState,
    ...
    {Encoded, NewEncodeState} = cow_hpack:encode(Headers, EncodeState),
